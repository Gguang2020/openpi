...
class AstribotInputs(transforms.DataTransformFn):
    """Inputs for the Astribot policy.
    ...
    def __call__(self, data: dict) -> dict:
        data = _decode_astribot(data, adapt_to_pi=self.adapt_to_pi)
        ...
        if "cam_high" in in_images:
            ...
            base_image = np.zeros((720, 1280, 3), dtype=np.uint8)
            images = {
...
class AstribotOutputs(transforms.DataTransformFn):
    """Outputs for the Astribot policy."""
    ...
    def __call__(self, data: dict) -> dict:
        ...
        actions = np.asarray(data["actions"][:, 0:31])
        return {"actions": _encode_actions(actions, adapt_to_pi=self.adapt_to_pi)}

def _get_astribot_torso_right_head_joint(ori_joint) -> np.ndarray:
    """
...
def _get_astribot_torso_left_right_head_joint(ori_joint) -> np.ndarray:
    """
    Get the torso-left-head joint from the original joint.
    torso joint: 2,3,4,5
...
def _joint_flip_mask() -> np.ndarray:
    """Used to convert between Astribot and pi joint angles."""
...
def _gripper_to_angular(value):
    # Astribot transforms the gripper positions into a linear space. The following code
    # reverses this transformation to be consistent with pi0 which is pretrained in
    # angular space.
    #
    # These values are coming from the Astribot code:
...
def _gripper_from_angular(value):
    # Convert from the gripper position used by pi0 to the gripper position that is used by Astribot.
...
def _decode_astribot(data: dict, *, adapt_to_pi: bool = False) -> dict:
    ...
    if state.shape[0] == 31:
        ...
        print("torso_arm_gripper_head  state shape is not 31")
...
def _decode_state(state: np.ndarray, *, adapt_to_pi: bool = False) -> np.ndarray:
    if adapt_to_pi:
        ...
        # Reverse the gripper transformation that is being applied by the Astribot runtime.
